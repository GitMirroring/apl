...
//════════════════════════════════════════════════════════════════════════════
// bif worker functions (static)
//════════════════════════════════════════════════════════════════════════════

extern const APL_Integer int_factorials[];    // defined in IntCell.cc
extern const APL_Float   float_factorials[];  // defined in IntCell.cc
enum { int_fact_max_w   = 21  };   // entries in int_factorials[]
enum { float_fact_max_w = 171 };   // entries in float_factorials[]
...
//────────────────────────────────────────────────────────────────────────────
// dyadic workers — a = APL left operand (A), b = APL right operand (B)
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_add_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_add_ff(Cell * Z, APL_Float a, APL_Float b)
{
   return FloatCell::zF(Z, a + b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_add_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
   return ComplexCell::zC(Z, a + b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_subtract_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
const APL_Integer z = a - b;
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_subtract_ff(Cell * Z, APL_Float a, APL_Float b)
{
   return FloatCell::zF(Z, a - b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_subtract_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
   return ComplexCell::zC(Z, a - b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_multiply_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_multiply_ff(Cell * Z, APL_Float a, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_multiply_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_divide_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
   if (b == 0)
      {
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_divide_ff(Cell * Z, APL_Float a, APL_Float b)
{
   if (b == 0.0)
      {
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_divide_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_power_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_power_fi(Cell * Z, APL_Float a, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_power_ci(Cell * Z, APL_Complex a, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_power_ff(Cell * Z, APL_Float a, APL_Float b)
{
   if (a == 0.0)
      {
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_power_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_logarithm_ff(Cell * Z, APL_Float a, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_logarithm_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
   // a = base (A), b = argument (B = this)
   //
   if (b == a)   return IntCell::z1(Z);
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_maximum_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
   return IntCell::zI(Z, a >= b ? a : b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_maximum_ff(Cell * Z, APL_Float a, APL_Float b)
{
   return NumericCell::zV(Z, a >= b ? a : b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_maximum_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
   if (!nc_near_real(a))   return E_DOMAIN_ERROR;
   if (!nc_near_real(b))   return E_DOMAIN_ERROR;
   return NumericCell::zV(Z, a.real() >= b.real() ? a.real() : b.real());
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_minimum_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
   return IntCell::zI(Z, a <= b ? a : b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_minimum_ff(Cell * Z, APL_Float a, APL_Float b)
{
   return NumericCell::zV(Z, a <= b ? a : b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_minimum_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
   if (!nc_near_real(a))   return E_DOMAIN_ERROR;
   if (!nc_near_real(b))   return E_DOMAIN_ERROR;
   return NumericCell::zV(Z, a.real() <= b.real() ? a.real() : b.real());
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_residue_ii(Cell * Z, APL_Integer a, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_residue_ff(Cell * Z, APL_Float a, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_residue_cc(Cell * Z, APL_Complex a, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_circle_fun_c(Cell * Z, APL_Integer fun, APL_Complex b)
{
   IntCell::z0(Z);
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_circle_fun_inverse_c(Cell * Z, APL_Integer fun, APL_Complex b)
{
   IntCell::z0(Z);
...
//────────────────────────────────────────────────────────────────────────────
// monadic workers — b = APL right operand (B = this)
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_ceiling_i(Cell * Z, APL_Integer b)
{
   return IntCell::zI(Z, b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_ceiling_f(Cell * Z, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_ceiling_c(Cell * Z, APL_Complex b)
{
...
const APL_Float D  = Dr + Di;
   if (D < 1.0)     return NumericCell::zV(Z, cr, ci);
   if (Di > Dr)     return NumericCell::zV(Z, cr, ci - 1.0);
   else             return NumericCell::zV(Z, cr - 1.0, ci);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_floor_i(Cell * Z, APL_Integer b)
{
   return IntCell::zI(Z, b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_floor_f(Cell * Z, APL_Float b)
{
   if (b >= LARGE_INT)   return FloatCell::zF(Z, b);
   if (b <= SMALL_INT)   return FloatCell::zF(Z, b);
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_floor_c(Cell * Z, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_conjugate_i(Cell * Z, APL_Integer b)
{
   return IntCell::zI(Z, b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_conjugate_f(Cell * Z, APL_Float b)
{
   return FloatCell::zF(Z, b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_conjugate_c(Cell * Z, APL_Complex b)
{
   return ComplexCell::zC(Z, conj(b));
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_direction_i(Cell * Z, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_direction_f(Cell * Z, APL_Float b)
{
   if (b > 0.0)   return IntCell::z1(Z);
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_direction_c(Cell * Z, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_exponential_i(Cell * Z, APL_Integer b)
{
   return FloatCell::zF(Z, exp(APL_Float(b)));
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_exponential_f(Cell * Z, APL_Float b)
{
   return FloatCell::zF(Z, exp(b));
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_exponential_c(Cell * Z, APL_Complex b)
{
   return ComplexCell::zC(Z, complex_exponent(b));
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_factorial_i(Cell * Z, APL_Integer b)
{
   if (b < 0)                    return E_DOMAIN_ERROR;
   if (b < int_fact_max_w)       return IntCell::zI(Z, int_factorials[b]);
   if (b < float_fact_max_w)     return FloatCell::zF(Z, float_factorials[b]);
   return E_DOMAIN_ERROR;
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_factorial_f(Cell * Z, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_factorial_c(Cell * Z, APL_Complex b)
{
   if (nc_near_real(b))   return bif_factorial_f(Z, b.real());
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_magnitude_i(Cell * Z, APL_Integer b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_magnitude_f(Cell * Z, APL_Float b)
{
   return FloatCell::zF(Z, b < 0.0 ? -b : b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_magnitude_c(Cell * Z, APL_Complex b)
{
   return FloatCell::zF(Z, abs(b));
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_nat_log_i(Cell * Z, APL_Integer b)
{
   if (b == 0)   return E_DOMAIN_ERROR;
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_nat_log_f(Cell * Z, APL_Float b)
{
   if (b == 0.0)   return E_DOMAIN_ERROR;
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_nat_log_c(Cell * Z, APL_Complex b)
{
   return ComplexCell::zC(Z, log(b));
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_negative_i(Cell * Z, APL_Integer b)
{
   if (uint64_t(b) == 0x8000000000000000ULL)
      return FloatCell::zF(Z, -APL_Float(b));
   return IntCell::zI(Z, -b);
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_negative_f(Cell * Z, APL_Float b)
{
   return FloatCell::zF(Z, -b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_negative_c(Cell * Z, APL_Complex b)
{
   return ComplexCell::zC(Z, -b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_pi_times_i(Cell * Z, APL_Integer b)
{
   return FloatCell::zF(Z, M_PI * b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_pi_times_f(Cell * Z, APL_Float b)
{
   return FloatCell::zF(Z, b * M_PI);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_pi_times_c(Cell * Z, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_pi_times_inverse_i(Cell * Z, APL_Integer b)
{
   return FloatCell::zF(Z, b / M_PI);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_pi_times_inverse_f(Cell * Z, APL_Float b)
{
   return FloatCell::zF(Z, b / M_PI);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_pi_times_inverse_c(Cell * Z, APL_Complex b)
{
const APL_Float pi(M_PI);
   return ComplexCell::zC(Z, b.real() / pi, b.imag() / pi);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_reciprocal_i(Cell * Z, APL_Integer b)
{
   switch(b)
      {
        case  0: return E_DOMAIN_ERROR;
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_reciprocal_f(Cell * Z, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_reciprocal_c(Cell * Z, APL_Complex b)
{
   // near-zero check
   //
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_roll_i(Cell * Z, APL_Integer b)
{
   if (b <= 0)   return E_DOMAIN_ERROR;
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_roll_f(Cell * Z, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_roll_c(Cell * Z, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_near_int64_t_i(Cell * Z, APL_Integer b)
{
   return IntCell::zI(Z, b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_near_int64_t_f(Cell * Z, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_near_int64_t_c(Cell * Z, APL_Complex b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_within_quad_CT_i(Cell * Z, APL_Integer b)
{
   return IntCell::zI(Z, b);
}
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_within_quad_CT_f(Cell * Z, APL_Float b)
{
...
//────────────────────────────────────────────────────────────────────────────
ErrorCode
NumericCell::bif_within_quad_CT_c(Cell * Z, APL_Complex b)
{
