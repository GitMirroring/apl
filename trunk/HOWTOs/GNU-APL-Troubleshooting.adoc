...
sub-directories, has more than 159,000 lines of code. Finding faults in
...
following 4 categories of increasing severity:
...
By making assertions all over the place in the source code, the programmer
...
Assert() and Assert1(). As of this writing there are more than 600 Assert()
and more than 120 Assert1() macros spread across the *GNU APL* source code.
...
address numbers on the left side of the C/Cpp stack dump are process
...
/home/eedjsa/apl-2.0 to fix this
...
----
...
Some programming errors bypass all built-in debugging tools of GNU APL and
...
currently over 50 Groups called *logging facilities*. Each logging facility
can be turned ON or OFF which causes Log() macros belonging to the facility
to print or not to print its information.

The large (and increasing) number of logging facilities causes the tests,
...
Many of the currently over 50 logging facilities serve a particular purpose
...
Now let's see how *LOC* works. Consider the first *LOC* on line *10* of
file *test.cc*. Then:
...
when) a particular source code line is reached. To answer this
