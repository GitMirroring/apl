...
* workspace-to-workspace variables which create a communication link between
  two running APL workspaces (in different processes on the same machine).
  This is a peer-to-peer communication.
...
*Handle←⎕FIO[24] Bs* (aka. popen(Bs, "r")) then the handle returned to APL
